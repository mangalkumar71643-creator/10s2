...
/**
 * "Dragon Tiger": one card to Dragon, one to Tiger, higher rank wins (Ace
 * low, King high, suits don't rank). Shared rounds on fixed wall-clock
 * slots, same shape as 7 Up Down: betting, then dealing, then the reveal.
 */
export const ROUND_SECONDS = 20;
/** Bets are accepted while less than this much of the round has passed. */
export const BET_SECONDS = 13;
/** Both cards are revealed this far into the round (the gap is the deal). */
export const RESULT_AT_SECONDS = 15;

export const AREAS = ["DRAGON", "TIE", "TIGER", "SUITED_TIE"] as const;
...
/** Chance of each bet winning when both cards are drawn independently from
 * a full 52-card deck (an infinite shoe): a tie is 1 in 13, a suited tie
 * (same rank and suit) 1 in 52, and Dragon / Tiger split the rest. */
function winChance(area: Area): number {
  if (area === "TIE") return 1 / 13;
  if (area === "SUITED_TIE") return 1 / 52;
  return 6 / 13;
}

/** Total return per unit staked (stake included), so multiplier * P(win) =
 * the shared rtp on every bet. Dragon / Tiger bets lose on a tie. Rounded
...
export function winnerOf(dragon: number, tiger: number): Winner {
  const d = cardOf(dragon).rank;
  const t = cardOf(tiger).rank;
  if (d === t) return "TIE";
  return d > t ? "DRAGON" : "TIGER";
}

export function areaWins(area: Area, dragon: number, tiger: number): boolean {
  const winner = winnerOf(dragon, tiger);
  if (area === "SUITED_TIE") return dragon === tiger;
  return area === winner;
}

/** Each card is its own HMAC draw on the round's server seed, so the
 * result is fixed the moment the round is created (commit-reveal: only the
 * seed hash is public until the cards are shown). */
export function cardsFor(serverSeed: string, periodNumber: string): [number, number] {
  const dragon = Math.floor(fairRandomFloat(serverSeed, periodNumber, 0) * 52);
  const tiger = Math.floor(fairRandomFloat(serverSeed, periodNumber, 1) * 52);
  return [dragon, tiger];
}
...
type RoundRow = Prisma.DragonTigerRoundGetPayload<Record<string, never>>;
...
async function getOrCreateRound(now: Date): Promise<RoundRow> {
  ...
  const existing = await prisma.dragonTigerRound.findUnique({ where: { periodNumber } });
  if (existing) return existing;

  const serverSeed = generateServerSeed();
  const [dragonCard, tigerCard] = cardsFor(serverSeed, periodNumber);
  try {
    return await prisma.dragonTigerRound.create({
      data: {
        periodNumber,
        startTime,
    ...
    return prisma.dragonTigerRound.findUniqueOrThrow({ where: { periodNumber } });
  }
}
...
async function settleRound(round: RoundRow) {
  const pending = await prisma.dragonTigerBet.findMany({ where: { roundId: round.id, status: "PENDING" } });

  for (const bet of pending) {
    const won = areaWins(bet.area as Area, round.dragonCard, round.tigerCard);
    const payout = won ? Math.min(round2(Number(bet.amount) * Number(bet.multiplier)), env.games.maxPayout) : 0;
    await prisma.$transaction(async (tx) => {
      const claimed = await tx.dragonTigerBet.updateMany({
  ...
  await prisma.dragonTigerRound.updateMany({ where: { id: round.id, settled: false }, data: { settled: true } });
}
...
async function settleDueRounds(now: Date) {
  const due = await prisma.dragonTigerRound.findMany({
...
function toView(round: RoundRow, now: Date) {
  ...
  return {
    ...
    dragon: revealed ? cardOf(round.dragonCard) : null,
    tiger: revealed ? cardOf(round.tigerCard) : null,
    winner: revealed ? winnerOf(round.dragonCard, round.tigerCard) : null,
    suitedTie: revealed ? round.dragonCard === round.tigerCard : null,
...
export function getDragonTigerConfig() {
  return {
 ...
 * within maxPayout. */
export async function placeDragonTigerBets(userId: string, bets: BetInput[]) {
  ...
  const created = await prisma.$transaction(async (tx) => {
    ...
    const mine = await tx.dragonTigerBet.findMany({
    ...
    for (const b of bets) {
      rows.push(
        await tx.dragonTigerBet.create({
...
export async function cancelDragonTigerBets(userId: string, betIds?: string[]) {
  const now = new Date();
  const round = await getOrCreateRound(now);
  if (phaseFor(round, now) !== "BETTING") {
  ...
  return prisma.$transaction(async (tx) => {
    await tx.$queryRaw`SELECT id FROM "Wallet" WHERE "userId" = ${userId} FOR UPDATE`;
    const targets = await tx.dragonTigerBet.findMany({
...
export async function getMyRoundBets(userId: string, periodNumber?: string) {
  ...
  const round = periodNumber
    ? await prisma.dragonTigerRound.findUnique({ where: { periodNumber } })
  ...
  const bets = await prisma.dragonTigerBet.findMany({
...
export async function getDragonTigerHistory(limit = 100) {
  const rounds = await prisma.dragonTigerRound.findMany({
    where: { settled: true },
    orderBy: { startTime: "desc" },
    take: limit,
    select: { periodNumber: true, dragonCard: true, tigerCard: true, serverSeed: true, serverSeedHash: true },
  });
  return rounds.map((r) => ({
    periodNumber: r.periodNumber,
    dragon: cardOf(r.dragonCard),
    tiger: cardOf(r.tigerCard),
    winner: winnerOf(r.dragonCard, r.tigerCard),
    suitedTie: r.dragonCard === r.tigerCard,
    serverSeed: r.serverSeed,
    serverSeedHash: r.serverSeedHash,
  }));
}

export async function getMyDragonTigerBets(userId: string, limit = 50) {
  const bets = await prisma.dragonTigerBet.findMany({
    where: { userId, status: { not: "VOID" } },
    orderBy: { createdAt: "desc" },
    take: limit,
    include: { round: { select: { periodNumber: true, dragonCard: true, tigerCard: true, settled: true } } },
  });
  return bets.map(({ round, ...bet }) => ({
    ...bet,
    periodNumber: round.periodNumber,
    dragon: round.settled ? cardOf(round.dragonCard) : null,
    tiger: round.settled ? cardOf(round.tigerCard) : null,
  }));
}
