/**
 * "Cricket X": the Aviator crash engine with a cricket theme — the batter
 * hits, the multiplier climbs while the ball is in the air, and the round
 * ends when it's caught (the crash point). Same provably fair crash-point
 * formula, timing and money-safety rules as aviatorService.ts, on its own
 * tables so the two games keep separate rounds and history.
 */
...
async function createNextRound(bettingStartTime: Date) {
  ...
  try {
    return await prisma.cricketXRound.create({
    ...
    return prisma.cricketXRound.findUniqueOrThrow({ where: { periodNumber } });
  }
}
 ...
 * ball is caught, without waiting for the trailing result-pause to end. */
async function resolvePendingBets(roundId: string) {
  const round = await prisma.cricketXRound.findUniqueOrThrow({ where: { id: roundId } });
  const crashMultiplier = Number(round.crashMultiplier);
  const pendingBets = await prisma.cricketXBet.findMany({ where: { roundId, status: "PENDING" } });
  ...
  for (const bet of pendingBets) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const claimed = await tx.cricketXBet.updateMany({
...
async function settleRound(roundId: string) {
  ...
  await prisma.cricketXRound.update({ where: { id: roundId }, data: { settled: true } });
}
...
export async function ensureCurrentRound() {
  let round = await prisma.cricketXRound.findFirst({ orderBy: { bettingStartTime: "desc" } });
...
export async function placeCricketXBet(userId: string, amount: number, autoCashoutAt?: number) {
  ...
  const bet = await prisma.$transaction(async (tx) => {
    ...
    const existingCount = await tx.cricketXBet.count({ where: { roundId: round.id, userId, status: "PENDING" } });
    if (existingCount >= 2) {
    ...
    return tx.cricketXBet.create({ data: { roundId: round.id, userId, amount, autoCashoutAt: autoCashoutAt ?? null } });
  });

  return bet;
}

export async function cashOutCricketXBet(userId: string, betId: string) {
  const bet = await prisma.cricketXBet.findUnique({ where: { id: betId }, include: { round: true } });
  ...
  if (phase === "BETTING") throw new ApiError(400, "The ball hasn't been hit yet.");
  if (phase === "CRASHED") throw new ApiError(400, "Too late — the ball was caught.");
...
export async function getMyCurrentBet(userId: string) {
  const round = await ensureCurrentRound();
  return prisma.cricketXBet.findFirst({
...
export async function getHistory(limit = 30) {
  return prisma.cricketXRound.findMany({
    where: { settled: true },
...
export async function getMyBets(userId: string, limit = 50) {
  return prisma.cricketXBet.findMany({
    where: { userId },
    orderBy: { createdAt: "desc" },
    take: limit,
    include: {
...
export async function getRoundBets(periodNumber: string, limit = 100) {
  const round = await prisma.cricketXRound.findUnique({ where: { periodNumber }, select: { id: true } });
  if (!round) return [];
  const bets = await prisma.cricketXBet.findMany({
...
export function getCricketXConfig() {
  return {
