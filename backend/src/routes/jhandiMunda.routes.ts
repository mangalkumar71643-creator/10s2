...
import {
  ...
  getCurrentRoundView,
  getMyRoundBets,
  getMyJhandiMundaBets,
  getJhandiMundaConfig,
  getJhandiMundaHistory,
  placeJhandiMundaBets,
} from "../services/jhandiMundaService";

const router = Router();

router.get("/config", (_req, res) => {
  res.json(getJhandiMundaConfig());
});
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await getJhandiMundaHistory(limit));
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(201).json(await placeJhandiMundaBets(req.user!.userId, bets));
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await cancelJhandiMundaBets(req.user!.userId, betIds));
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await getMyJhandiMundaBets(req.user!.userId, limit));
  })
);
