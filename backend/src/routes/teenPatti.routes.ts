...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await getTeenPattiHistory(limit));
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(201).json(await placeTeenPattiBets(req.user!.userId, bets));
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await cancelTeenPattiBets(req.user!.userId, betIds));
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await getMyTeenPattiBets(req.user!.userId, limit));
  })
);
