...
export default function WithdrawScreen() {
  ...
  const isValidAmount = amountText.trim().length > 0 && amount >= MIN_WITHDRAW && amount <= coins;
  ...
  async function handleWithdraw() {
    ...
    try {
      ...
      await refreshWallet();
      Alert.alert('Withdrawal requested', `₹${amount.toLocaleString('en-IN')} will be sent to your account.`, [
        ...
        <View style={styles.amountCard}>
          <View style={styles.amountLabelRow}>
            ...
            <Text style={styles.amountLimit}>
              Min: ₹{MIN_WITHDRAW}   Max: ₹{Math.floor(coins)}
            </Text>
          </View>
